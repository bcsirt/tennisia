...
class VitesseSurface extends Model
{
    ...
    public function getImpactConditionsAttribute()
    {
        ...
        if ($this->sensible_humidite) $impacts[] = 'Humidité';
        if ($this->sensible_temperature) $impacts[] = 'Température';
        if ($this->sensible_altitude) $impacts[] = 'Altitude';
    ...
    {
        if ($this->valeur >= 80) return 'Très rapide';
        if ($this->valeur >= 60) return 'Rapide';
        if ($this->valeur >= 40) return 'Moyenne';
        if ($this->valeur >= 20) return 'Lente';
        return 'Très lente';
    }
    ...
    public function getCaracteristiquesPrincipalesAttribute()
    {
        ...
        if ($this->favorise_puissance) $caracteristiques[] = 'Puissance';
        if ($this->favorise_precision) $caracteristiques[] = 'Précision';
        if ($this->favorise_endurance) $caracteristiques[] = 'Endurance';
        if ($this->facilite_passing_shots) $caracteristiques[] = 'Passing shots';
    ...
    protected static function boot()
    {
        ...
        static::saving(function ($vitesse) {
            ...
            // Valeurs par défaut
            if ($vitesse->actif === null) $vitesse->actif = true;
            if ($vitesse->est_visible === null) $vitesse->est_visible = true;
        });
    }
}
